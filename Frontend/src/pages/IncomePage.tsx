...
import type { CreateIncomeRequest, Income, IncomeSummary, IncomeType } from '@/lib/types';
import { TrendingUp, BadgeDollarSign, Calendar } from 'lucide-react';
    ...
    <>
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <StatCard title="Total do Mês" value={summary.total} icon={TrendingUp} comparison={summary.comparisonPercentage} gradient="purple" />
          <StatCard title="Salário + Freelance" value={summary.salaryTotal + summary.freelanceTotal} icon={BadgeDollarSign} subtitle={`Salário: ${brl(summary.salaryTotal)} · Freelance: ${brl(summary.freelanceTotal)}`} />
          <StatCard title="Média Diária" value={summary.dailyAverage} icon={Calendar} subtitle="Por dia do mês" />
        </div>
      )}
      ...
      <Card>
        <CardHeader>
          <div>
            ...
            <CardDescription>{filtered.length} receita{filtered.length !== 1 ? 's' : ''}</CardDescription>
          </div>
        </CardHeader>
